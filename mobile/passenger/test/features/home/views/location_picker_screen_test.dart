...
RideHistoryItem _item(int i, {String? origin, String? destination}) => RideHistoryItem(
      id: 'r$i',
...
void main() {
  ...
  testWidgets('Triggers ride history load in initState when status is RideHistoryStatus.initial', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('Falls back to suggested transit points when history status is success but items are empty', (
    WidgetTester tester,
  ) async {
    final fakeHistory = _FakeRideHistoryNotifier(
      const RideHistoryListState(status: RideHistoryStatus.success, items: []),
    );
  ...
  testWidgets('Falls back to suggested transit points when history status is success but items have only empty addresses', (
    WidgetTester tester,
  ) async {
    final fakeHistory = _FakeRideHistoryNotifier(
  ...
  testWidgets('Displays RECENT PICKUPS with formatted and duplicate-filtered unique items capped at 6 for LocationSearchMode.pickup', (
    WidgetTester tester,
  ) async {
    final fakeHistory = _FakeRideHistoryNotifier(
      RideHistoryListState(
        ...
        items: [
          ...
          _item(8, origin: 'Manila Bay, Manila'), // 7th unique item, should be capped out
        ],
      ),
    );
  ...
  testWidgets('Displays RECENT DESTINATIONS with formatted and duplicate-filtered unique items capped at 6 for LocationSearchMode.destination', (
    WidgetTester tester,
  ) async {
    final fakeHistory = _FakeRideHistoryNotifier(
      RideHistoryListState(
        ...
        items: [
          ...
          _item(8, destination: 'Intramuros, Manila'), // 7th unique item, should be capped out
        ],
      ),
    );
    ...
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: MaterialApp(
          home: Scaffold(
            body: const LocationPickerScreen(mode: LocationSearchMode.destination),
          ),
        ),
      ),
    );
    ...
    expect(find.text('Recent Destination · Quezon City'), findsOneWidget);

    // Intramuros should not be present because it exceeds the limit of 6
    expect(find.text('Intramuros'), findsNothing);
  });

  testWidgets('Tapping fallback suggested item immediately pops resolved RideLocation', (
    WidgetTester tester,
  ) async {
    final fakeHistory = _FakeRideHistoryNotifier(
      const RideHistoryListState(status: RideHistoryStatus.success, items: []),
            ...
            body: Builder(
              builder: (context) {
                return ElevatedButton(
                  onPressed: () async {
                    poppedLocation = await Navigator.of(context).push<RideLocation>(
                      MaterialPageRoute(
    ...
    expect(poppedLocation!.address, equals('Central Business District Office, 120 Pine St'));
  });

  testWidgets('Tapping a recent address suggestions geocodes and pops resolved RideLocation', (
    WidgetTester tester,
  ) async {
    final fakeHistory = _FakeRideHistoryNotifier(
      RideHistoryListState(
  ...
  testWidgets('Displays error snackbar if geocoding fails on tapping suggestion', (
    WidgetTester tester,
  ) async {
    final fakeHistory = _FakeRideHistoryNotifier(
      RideHistoryListState(
        status: RideHistoryStatus.success,
        items: [
          _item(1, origin: 'Fail Location, Area'),
        ],
      ),
    );
    ...
    expect(find.text('Could not resolve address. Try again.'), findsOneWidget);
  });
}
