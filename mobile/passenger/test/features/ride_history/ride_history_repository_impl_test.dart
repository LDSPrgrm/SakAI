...
void main() {
  ...
  group('RideHistoryRepositoryImpl.getRideHistory', () {
    test('successfully fetches ride history and updates hasMore', () async {
      ...
      final mockApiResponse = UserRideListResponse((b) => b
        ..data.addAll([
          UserRideItem((r) => r
            ..id = 'r-1'
        ...
        ..pagination = PaginationMeta((p) => p
          ..currentPage = 1
          ..totalPages = 2
          ..totalItems = 21
          ..limit = 20).toBuilder());

      when(mockRidesApi.rideList(
        page: anyNamed('page'),
  ...
  group('RideHistoryRepositoryImpl.getRideDetail', () {
    test('successfully fetches ride detail', () async {
      final now = DateTime.now();
      final passenger = $UserProfile((b) => b
        ..id = 'u-1'
      ...
      final mockApiResponse = $RideResponse((r) => r
        ..id = 'r-1'
        ..status = RideStatus.completed
        ..passenger = passenger
        ..originAddress = 'Origin'
        ..destinationAddress = 'Destination'
        ..origin = LatLng((l) => l..lat = 1.0..lng = 1.0).toBuilder()
        ..destination = LatLng((l) => l..lat = 2.0..lng = 2.0).toBuilder()
        ..createdAt = now
        ..updatedAt = now
      );

      when(mockRidesApi.rideGet(rideId: 'r-1')).thenAnswer((_) async => Response(
            data: mockApiResponse,
            statusCode: 200,
            requestOptions: RequestOptions(path: ''),
          ));
    ...
    test('throws RideHistoryException when response data is null', () async {
      ...
      expect(
        ...
        throwsA(isA<RideHistoryException>().having((e) => e.userMessage, 'userMessage', 'Ride details not found.')),
      );
    });
  });
}
