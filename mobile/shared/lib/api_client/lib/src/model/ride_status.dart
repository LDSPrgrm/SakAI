...
class RideStatus extends EnumClass {
  ...
  /// Ride state machine: ``` created → requested → accepted → arrived → in_progress → payment_pending → completed      ↘          ↙       ↙                                 ↙        cancelled  (not from in_progress) ----------------- ``` Terminal states: `completed`, `cancelled` 
  @BuiltValueEnumConst(wireName: r'created')
  static const RideStatus created = _$created;
  /// Ride state machine: ``` created → requested → accepted → arrived → in_progress → payment_pending → completed      ↘          ↙       ↙                                 ↙        cancelled  (not from in_progress) ----------------- ``` Terminal states: `completed`, `cancelled` 
  ...
  /// Ride state machine: ``` created → requested → accepted → arrived → in_progress → payment_pending → completed      ↘          ↙       ↙                                 ↙        cancelled  (not from in_progress) ----------------- ``` Terminal states: `completed`, `cancelled` 
  @BuiltValueEnumConst(wireName: r'payment_pending')
  static const RideStatus paymentPending = _$paymentPending;
  /// Ride state machine: ``` created → requested → accepted → arrived → in_progress → payment_pending → completed      ↘          ↙       ↙                                 ↙        cancelled  (not from in_progress) ----------------- ``` Terminal states: `completed`, `cancelled` 
