...
| Architecture | Backend: Clean Architecture (domain / usecase / repository / delivery). Mobile: domain / data / presentation + shared package |
...
| `mobile/passenger/`, `mobile/driver/` | Apps; depend on `sakai_shared`; keep networking in `data/` when layers exist |
...
- Build a small `data/` module: connect to `SakaiApiEndpoints.webSocketUri(restBaseUrl, accessToken)`, parse `{ "event": "...", "payload": { ... } }`, switch on `event` with typed payloads where models exist in `sakai_api_client` (e.g. WS event schema models).
...
| Flutter: no raw `Dio` in widgets | Keeps presentation thin; use `data/` facades |
