...
test.describe("End-to-End Ride Flow", () => {
  test("should complete a ride from request to dropoff", async ({
    ...
    const driverSkip = driverPage.getByRole("button", { name: "Skip" });
    const driverSignInButton = driverPage.getByRole("button", { name: /Sign in/i });
    await Promise.race([
    ...
    await expect(driverSignInButton).toBeAttached({ timeout: 20000 });
    ...
    await driverSignInButton.click({ force: true });

    const onlineToggle = driverPage.locator("text=Start shift").first();
    await expect(onlineToggle).toBeAttached({ timeout: 30000 });
    ...
    await expect(saanKayoText).toBeAttached({ timeout: 30000 });
    ...
    const whereToButtonRole = passengerPage.getByRole("button", { name: "Where to?" });
    await whereToButtonRole.click({ force: true });
    await passengerPage.waitForTimeout(1000);
    const whereToInput = passengerPage.getByRole("textbox").last();
    await expect(whereToInput).toBeAttached({ timeout: 15000 });
