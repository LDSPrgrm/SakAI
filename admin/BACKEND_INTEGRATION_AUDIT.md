...
- **Phase 4c** — partial. Admin 🟢 polish bucket landed 2026-05-21: ✅ #19 (displayRole role-rename tests + rename-aware lookup), ✅ #21 (SAFareConfig sim coords now a 5-city Metro Manila preset selector — no more hardcoded `14.5995, 120.9842`), ✅ #29 (SAReports top "Export CSV" button now disabled until a report is picked, label reflects selection, PageHeader subtitle shows current pick), ✅ #30 (PermissionGrid `parsePermissions` surfaces unknown keys + SARoleManagement renders amber warning + `console.warn`), ✅ #37 (Template editor flags variables not present in backend-shipped body as Unknown, blocks save), ✅ #38 (every feature-flag toggle now goes through `ConfirmationModal`, not just `maintenance_mode`). Verification: `npx tsc --noEmit` clean, `npm test` 249/249. Still pending and out of admin scope: real SMS/email/push provider swap in `notifications.Dispatcher.send()`, SOS-trigger write-path on mobile, `migrate down` cycle exercise for 025/026/027, browser smoke test of Phase 4a/4b UI, and #25 SAPayments CSV export server-side ownership (backend work).
